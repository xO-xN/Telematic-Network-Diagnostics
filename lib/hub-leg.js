...
class HubLeg {
  ...
  constructor({
    ...
    // The relayed stats payload's site-level fields, frozen here for
    // #5 to fill (issue #4 relays them as "no data" until then).
    performerCount = () => 0,
    localWorst = () => null,
  }) {
  ...
  start() {
    ...
    this.socket.on("relay", (body) => {
      ...
      this.peers.set(body.from, {
        ...
        performerCount:
          typeof body.performerCount === "number" ? body.performerCount : 0,
        localWorst:
          typeof body.localWorst === "string" ? body.localWorst : null,
        lastSeenAt: this.now(),
      });
    });
  ...
  // (issue #4): own hub-leg quality + numbers, plus the site-level
  // fields #5 will start filling.
  announceStats() {
  ...
  ownSnapshot() {
    ...
    return {
      ...
      // The site-level fields relayed to the room (#5 fills them with
      // real data; the own card reads the same values it announces).
      performerCount: this.performerCount(),
      localWorst: this.localWorst(),
      summary: {
        ...summary,
