...
// #10), which also appends to the SITE-level event log (the card's own
// log dies with the card, but the exit itself must stay traceable).
class LocalSession {
  constructor({
    windowSize = 10,
    warmupCycles = 2,
  ...
  snapshot(now = 0) {
    ...
    return {
      ...
      ...this.siteSummary(),
      clients,
      // Site-level events (voluntary exits) — the only local event
      // history that outlives a deleted card.
