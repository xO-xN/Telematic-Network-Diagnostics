...
//   MetricsCollector           — per-performer sliding window of
//                                phase-stamped RTT samples, timeouts,
//                                load-scoped jitter and RTT percentiles,
...
// The phase tokens the server's phase timer passes to setPhase (server.js
// enterBurstPhase / enterCalmPhase — literals, same as the relay body's
// `probing` field). The collector stamps its samples with the burst token.
const BURST_PHASE = "burst";
...
// Per-performer metrics: a sliding window of RTT samples — each stamped
// with the burst/calm phase it was measured in — plus timeout totals and
// the consecutive-timeout streak that drives the Red rule, the lifetime
// probe counters (acks/timeouts → loss rate) and the per-burst-window
// timeout rate (frozen when a burst window completes).
//
// The RTT percentiles and jitter are load-scoped (LND's fix, ported): once
// a performer has any burst sample, they are computed from burst samples
// only — the condition a show actually runs under. The calm phase probes
// at 1 Hz, and every idle second lets Wi-Fi power save (or a parked
// browser tab) add its wake-up delay to the next probe, so calm RTTs
// describe the idle pattern, not the link: measured unscoped, the
// burst→calm RTT step shows up as a ~4 s square wave on "stability" and
// pins the card Yellow through hysteresis. Unphased use (no phase stamps
// yet) keeps the legacy whole-window math.
class MetricsCollector {
  ...
  record(rttMs, processingMs = null, phase = null) {
    ...
    if (phase === BURST_PHASE && !switchesPhase) {
      this.seenLoad = true;
    }
  ...
  burstIndices() {
    ...
    for (let i = 0; i < this.samples.length; i += 1) {
      if (this.phases[i] === BURST_PHASE && this.jitterEligible[i]) {
        indices.push(i);
      }
    }
  ...
  // only. Calm-phase acks alone are not evidence about the show
  // condition, so a performer that joins mid-calm stays Gray until its
  // first burst.
...
// cycles and until the first load (burst) sample exists; recovery from
// Red/Yellow to Green needs `hysteresisCycles` consecutive good cycles
// (any bad cycle resets the counter); worsening is instant.
class StatusMachine {
...
// one — the server calls it on the performer's voluntary exit (issue
// #10) and on the monitor's「x」removal (issue #13), both of which also
// append to the SITE-level event log (the card's own log dies with the
// card, but the deletion itself must stay traceable).
class LocalSession {
  constructor({
    windowSize = 10,
    warmupCycles = 2,
    ...
    this.siteEvents = []; // { type, client, at } — outlives the cards
  }
  ...
  // Voluntary exit (issue #10): the card is deleted outright — the
  // metrics, the status machine and the card's own event log all go,
  // and the site summary stops counting the leg. The exit itself is
  // appended to the site-level log so the monitor can still trace it.
  // The monitor's「x」(issue #13) deletes through the same method with
  // its own `type` — the log words say WHO deleted the client
  // ("client N 退出（performer）" vs "client N 移除（monitor）").
  removeClient(id, now = 0, type = shared.localEvents.left) {
    if (!this.clients.delete(id)) {
      return;
    }

    this.siteEvents.push({ type, client: id, at: now });

    if (this.siteEvents.length > MAX_EVENTS) {
      this.siteEvents.shift();
    }
  }
  ...
  recordAck(id, rttMs, processingMs = null) {
    ...
    if (entry) {
      // The sample carries the phase the cycle is currently in; the
  ...
  snapshot(now = 0) {
    ...
    return {
      ...
      ...this.siteSummary(),
      clients,
      // Site-level events (deletions: voluntary exits, monitor
      // removals) — the only local event history that outlives a
      // deleted card.
      events: this.siteEvents.map((event) => ({
        type: event.type,
        client: event.client,
        at: event.at,
        agoMs: now ? Math.max(0, now - event.at) : 0,
